#include "system_analyzers/gps_analyzer.h"

PLUGINLIB_EXPORT_CLASS(diagnostic_aggregator::GPSAnalyzer,
...
{
    GPSAnalyzer::GPSAnalyzer() : level_(diagnostic_msgs::DiagnosticStatus::OK) {}

    bool GPSAnalyzer::init(const std::string base_path,
                           const ros::NodeHandle &n)
    {
        ...
        ros::NodeHandle nh = n;
        status_publisher_ = nh.advertise<status_msg>("/gps_status", 1);

        return true;
    }

    std::vector<diagnostic_msgs::DiagnosticStatusPtr> GPSAnalyzer::report()
    {
