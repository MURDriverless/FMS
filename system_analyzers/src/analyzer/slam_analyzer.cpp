#include "system_analyzers/slam_analyzer.h"

PLUGINLIB_EXPORT_CLASS(diagnostic_aggregator::SLAMAnalyzer,
...
{
    SLAMAnalyzer::SLAMAnalyzer() : level_(diagnostic_msgs::DiagnosticStatus::OK) {}

    bool SLAMAnalyzer::init(const std::string base_path,
                            const ros::NodeHandle &n)
    {
        ...
        ros::NodeHandle nh = n;
        status_publisher_ = nh.advertise<status_msg>("/slam_status", 1);

        return true;
    }

    std::vector<diagnostic_msgs::DiagnosticStatusPtr> SLAMAnalyzer::report()
    {
